...
class AttachmentsTest : ApplicationTestBase() {
    ...
    fun testUpload() = testApplication {
        ...
        val file1 = application.inject<FileStore>().read(Id.Factory.decode(uploadItems.items[0].id))
        assertNotNull(file1)
        assertEquals(file1Contents, file1Contents)
        val file2 = application.inject<FileStore>().read(Id.Factory.decode(uploadItems.items[1].id))
        assertNotNull(file2)
    ...
    fun testAttach() = testApplication {
        ...
        val file1 = application.inject<FileStore>().read(Id.Factory.decode(attachActions[0].id!!))
        assertNotNull(file1)
        assertEquals(file1Contents, file1Contents)
        val file2 = application.inject<FileStore>().read(Id.Factory.decode(attachActions[1].id!!))
        assertNotNull(file2)
        assertEquals(file2Contents, file2Contents)
    }
}